...
const timelineSchema = suitabilitySchema.pick({ timeline: true });
...
export default function TimelineForm() {
  ...
  const form = useForm({
    defaultValues: {
      timeline: (formData.timeline ?? "") as SuitabilityFormValues["timeline"],
    },
    validators: {
      onSubmit: timelineSchema,
    },
    onSubmit: async ({ value }) => {
      setFormData({ ...formData, ...value });
      window.location.href = "/suitability-check/goals";
    },
  });

  return (
    <div className="mx-auto max-w-3xl space-y-6 px-4 py-8">
      <FormProgress currentStep={2} totalSteps={11} />
      ...
      >
            ...
            return (
              <FieldSet>
                ...
                <RadioGroup
                  ...
                  onValueChange={(v) =>
                    field.handleChange(v as SuitabilityFormValues["timeline"])
                  }
        ...
        <div className="mt-8 flex items-center justify-between gap-4">
          ...
          <form.Subscribe selector={(state) => !!state.values.timeline}>
            {(canProceed) => (
